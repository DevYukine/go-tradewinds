...
<template>
  <div class="flex flex-col min-h-screen bg-slate-950 text-slate-100">
    ...
    <header class="bg-slate-900 border-b border-slate-700/50 sticky top-0 z-50">
      <div class="max-w-[2400px] mx-auto px-8 h-16 flex items-center justify-between">
        <!-- Left: Logo + Nav -->
        <div class="flex items-center gap-8">
          <NuxtLink to="/" class="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
            <Icon name="mdi:sail-boat" class="text-emerald-400 text-3xl" />
            <span class="text-xl font-bold text-slate-100 hidden sm:inline">Tradewinds</span>
          </NuxtLink>
        ...
        <div class="flex items-center gap-5">
          ...
          <!-- Rate Limit Gauge -->
          <div v-if="rateLimit" class="flex items-center gap-3">
    ...
    <!-- Main Content -->
    <main class="flex-1">
      <div class="max-w-[2400px] mx-auto px-8 py-8">
        <slot />
      </div>
    </main>
    ...
    <footer class="bg-slate-900/50 border-t border-slate-700/30 mt-auto">
      <div class="max-w-[2400px] mx-auto px-8 py-4 flex items-center justify-between text-sm text-slate-600">
